...
namespace gfx {
    ...
    public:
        ...
        virtual std::vector<std::uint32_t>& Render(std::vector<std::uint32_t>& buffer, std::uint32_t buffer_width, std::uint32_t buffer_height, 
            const math::vec4f& camera_pos, const math::vec4f& camera_dir, float FOV) const noexcept = 0;
        virtual gfx::Color CastRay(const gfx::Ray& ray, float& min_dist) const noexcept = 0;
